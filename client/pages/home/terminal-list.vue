    ...
    <el-scrollbar class="scroll-wrapper page-list-wrapper">
      <el-form :inline="true" label-position="right">
        <el-form-item label="终端信息：">
          <el-input v-model.trim="searchVal" placeholder="搜索终端名称或ID" clearable
            @keyup.enter.native="getAppList"></el-input>
        </el-form-item>
        <el-form-item label="终端创建时间：">
          <el-date-picker v-model="startTime" type="date" value-format="yyyy-MM-dd" :picker-options="beginOptions"
            placeholder="开始时间"> </el-date-picker>
          <span style="padding: 0 10px">至</span>
          <el-date-picker v-model="endTime" type="date" value-format="yyyy-MM-dd" :picker-options="endOptions"
          ...
          <el-table-column type="expand">
            <template slot-scope="props">
              <el-table :data="props.row.terminalList" style="width: 100%">
                ...
                <el-table-column prop="freeSpace" label="使用空间" align="center">
                  <template slot-scope="scope">
                      <div style="min-width: 120px;">
                        <el-progress :stroke-width="8" :show-text="false"  status="success" :text-inside="true"  :percentage="scope.row.percentage"></el-progress>
                    <div>{{ scope.row.freeSpaceData }}</div>
                      </div>
  ...
  methods: {
    ...
    detailTerminal(item) {
      ...
      this.$API.terminalDetailGet({ terminalId: item.terminalId }).then(response => {
        ...
        this.terminalDetailForm.totalSpace = this.changeFileSize(this.terminalDetailForm.totalSpace,2)
        this.terminalDetailForm.freeSpace = this.changeFileSize(this.terminalDetailForm.freeSpace,2)
      });
    },
    changeFileSize(limit,fix) {
      let size = "";
      if (limit < 1024) {                            //小于1KB，则转化成B
        size = limit.toFixed(fix) + "B"
      } else if (limit < 1024 * 1024) {            //小于1MB，则转化成KB
        size = (limit / 1024).toFixed(fix) + "KB"
      } else if (limit < 1024 * 1024 * 1024) {        //小于1GB，则转化成MB
        size = (limit / (1024 * 1024)).toFixed(fix) + "MB"
      } else {                                            //其他转化成GB
        size = (limit / (1024 * 1024 * 1024)).toFixed(fix) + "GB"
      }
      ...
      this.$API.terminalDetailListGet(params).then(res => {
        ...
        res.data.forEach(element => {
          element.terminalList.forEach(cru => {
            cru.percentage = Math.floor((cru.freeSpace / cru.totalSpace) * 100) || 100
            cru.freeSpaceData = this.changeFileSize(cru.freeSpace,1) + "/" + this.changeFileSize(cru.totalSpace,1)
            // cur.freeSpaceData = '112.09/119.02'
          })
        });
